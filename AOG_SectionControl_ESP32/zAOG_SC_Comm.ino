			...
			if (incomSentenceDigit == 3) {
				...
				{//>4	
					if (incomSentenceDigit == (SentenceFromAOGLength + 5)) { //sentence complete Length: + 4 byte header + 1 length + 1 CRC - 1 (starting at 0) 
						...
						if (Set.debugmodeDataFromAOG) {
              Serial.print("from AGO ");
							for (byte b = 0; b <= (SentenceFromAOGLength + 5); b++) {
                char bStr[10];
                //sprintf(bStr,"%03d ", SentenceFromAOG[b]);
								Serial.printf("%03d ", SentenceFromAOG[b]); // Serial.print(" ");
							}
							Serial.print("  ");
						}
						...
						if (isSteerDataFound) {
							...
							if (Set.debugmodeDataFromAOG) {
								...
								Serial.print(" SectGr[1]: "); Serial.print(SectGrFromAOG[1]); Serial.print(Debug_ln);
							}
						}
						else {

...
//#if HardwarePlatform == 0
	...
	for (int i = 0; i < Set.SectNum; i++)
	{
		if (Set.SectSWInst) {
			if (Set.SectSW_PIN[i] < 255) {
			  //if (Set.SectSW_PIN[i] < 0x80) {
			    _pinMode(Set.SectSW_PIN[i], INPUT_PULLUP);// }
        /*else {
          _digitalWrite(Set.SectSW_PIN[i] ,LOW);
          _pinMode(Set.SectSW_PIN[i], INPUT);          
        } */      
		} else { Serial.print("no PIN set for section switch #"); Serial.println(i+1); }
			if (Set.Relay_PIN[i] < 255) { _pinMode(Set.Relay_PIN[i], OUTPUT); }
			else { Serial.print("no PIN set for section relay #"); Serial.println(i+1); }
		}
	}
	if (Set.SectSWInst) {
		if (Set.SectAutoManSW_PIN < 255) { 
      if (Set.SectAutoManSW_on == 0 ) {
        _pinMode(Set.SectAutoManSW_PIN, INPUT_PULLUP); 
      } else {
        _pinMode(Set.SectAutoManSW_PIN, INPUT);   
      }
		} else { Serial.println("no PIN set for Auto/Manual toggle switch / buttons"); }
	}
	if (Set.SectMainSWType != 0) {
		if (Set.SectMainSW_PIN < 255) {
      _pinMode(Set.SectMainSW_PIN, INPUT_PULLUP);
		  if (Set.SectMainSWType == 4) {
		    _pinMode(Set.SectMainSW_PIN2, INPUT_PULLUP); 
		  }
	  }	else { Serial.println("no PIN set for Main ON/OFF toggle switch / buttons"); }
	}
	if (Set.RateSWLeftInst) {
		if (Set.RateSWLeft_PIN < 255) { _pinMode(Set.RateSWLeft_PIN, INPUT_PULLUP); }
		else { Serial.println("no PIN set for left Rate +/- left"); }
	}
	if (Set.RateSWRightInst) {
		if (Set.RateSWRight_PIN < 255) { _pinMode(Set.RateSWRight_PIN, INPUT_PULLUP); }
	...
	if (Set.FlowDirLeft_PIN < 255) { _pinMode(Set.FlowDirLeft_PIN, OUTPUT); }
	if (Set.FlowPWMLeft_PIN < 255) { _pinMode(Set.FlowPWMLeft_PIN, OUTPUT); }
	if (Set.FlowDirRight_PIN < 255) { _pinMode(Set.FlowDirRight_PIN, OUTPUT); }
	if (Set.FlowPWMRight_PIN < 255) { _pinMode(Set.FlowPWMRight_PIN, OUTPUT); }

	if (Set.Relais_MainValve_PIN < 255) { _pinMode(Set.Relais_MainValve_PIN, OUTPUT); }

	RelayOUT[0] = 0;
	RelayOUT[1] = 0;
	SetRelays();

	if (Set.LEDWiFi_PIN != 255) { _pinMode(Set.LEDWiFi_PIN, OUTPUT); }

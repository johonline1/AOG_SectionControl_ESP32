...
struct set {
	...
	uint8_t DataTransVia = 9;		//transfer data via 0 = USB / 7 = WiFi / 10 = Ethernet UDP / 9 = WT32_Eth01 Etheret
	...
	char ssid1[24] = "AGO_Hotspot";	// WiFi network Client name
	char password1[24] = "";                // WiFi network password//Accesspoint name and password
	char ssid2[24] = "AGO_Hotspot1";			// WiFi network Client name
	char password2[24] = "";                // WiFi network password//Accesspoint name and password
	char ssid3[24] = "AGO_Hotspot2";    // WiFi network Client name
	...
	byte timeoutWebIO = 255;//min  		    // time (min) afterwards webinterface is switched off	
	...
	byte Eth_mac[6] = { 0x90,0xA2,0xDA,0x10,0xB3,0x1C };
...
//Example2: PWM valve, with flowmeter all controled by AOG:   RateControl..Equiped = true; RateSW..Equiped = true; RateControlPWM = true;	
	...
	uint8_t RateSWLeft_PIN = 255;			// Rate +/- switch (ON)-0-(ON) toggle switch (GND)- 1.75V - (3,3V)
	...
	uint8_t Relay_PIN[16] = { 0x80,0x81,0x82,0x83,0x84,0x85,0x86,255,255,255,255,255,255,255,255,255 };  //GPIOs of ESP32 OUT to sections of sprayer HIGH/3.3V = ON
	uint8_t Relais_MainValve_PIN = 0x90;		// PIN for Main fluid valve 255 = unused
	...
	uint8_t SectSW_PIN[16] = { 0x8F,0x8E,0x8D,0x8C,0x8B,0x8A,0x89,255,255,255,255,255,255,255,255,255 };//section switches to GPIOs of ESP32 GND = section off, open/+3.3V section auto/on
 
	uint8_t	SectMainSWType = 6;				// 0 = not equiped 1 = (ON)-OFF-(ON) toggle switch or push buttons 2 = connected to hitch level sensor 3 = inverted hitch level sensor 4 = digital toggle buttons
	uint16_t	HitchLevelVal = 2000;		// Value for hitch level: switch AOG section control to Auto if lower than... ESP:2000 nano 500
 	uint8_t	SectMainSW_PIN = 0x87;			// ESP32 to AOG Main auto toggle switch open=nothing/AOG button GND=OFF +3,3=AOG Auto on	OR connected to hitch level sensor	
  uint8_t SectMainSW_PIN2 = 255;      // ESP32 to AOG Main auto toggle switch open=nothing/AOG button GND=OFF +3,3=AOG Auto on  OR connected to hitch level sensor  

  uint8_t SectAutoManSW_PIN = 0x88;     // Main Auto/Manual switch 39:!!no internal pullup!!
  uint8_t SectAutoManSW_on = 1;     // Main Auto/Manual  on 1 = high = used with pullup, 0 = low = pulldown 
  ...
  uint8_t PCF_SDA  = 33;          //  Wire SDA
  uint8_t PCF_SCL  = 32;          //  Wire SCL
...
byte debugmodeSwitchesBak = 1;  //0 = false 1 = true 3 = not used
...
void setup()
{
  ...
  Wire.begin(Set.PCF_SDA, Set.PCF_SCL);
	delay(50); 
	scanPCF(&Wire);
	Serial.println();
	//get EEPROM data
	...
	if (Set.aogVersion == 17) {
		...
		SCToAOGOld[4] = SCDataSentenceToAOGLength - 6;
...
//must run, also if nothing changed on switches, to keep GUI syncron (important if a section switch is off)
    ...
    //if (Set.debugmode) Serial.print ("\r");

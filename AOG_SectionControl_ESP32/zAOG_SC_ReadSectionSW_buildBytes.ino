void SectSWRead() {
  ...
  //read all switches
  if (Set.debugmodeSwitches) Serial.print("Section sw.:[");
  for (int i = 0; i < Set.SectNum; i++) {
    if (Set.SectSW_PIN[i] < 255) {
      SectSWVal[i] = _digitalRead(Set.SectSW_PIN[i]);
      //invert?
    ...
    if (Set.debugmodeSwitches) {
      /*Serial.print(i + 1); Serial.print("["); */ Serial.print(SectSWVal[i]);  //Serial.print("]");
    }
  }
  ...
  //AUTO / MAN SW
  AutoSWVal = _digitalRead(Set.SectAutoManSW_PIN);
  if (Set.SectAutoManSW_on == 0) AutoSWVal = !AutoSWVal;
  if (Set.debugmodeSwitches) {
    Serial.print("] Auto/Man:");
    Serial.print(AutoSWVal);
  }
  ...
  if (SectAuto != SectAutoOld) {
    ...
    if (SectSWcurrentTime > SectAutoSWTime + SectSWDelayTime) {
      SectAutoSWpressed = false;
      SectAutoOld = SectAuto;
    }
  }
  ...
  if (Set.SectMainSWType != 0) {
      if (Set.debugmodeSwitches) {
        Serial.print (" MainTyp:");     
        Serial.print (Set.SectMainSWType);
        Serial.print (" = ");     
        Serial.print (_digitalRead(Set.SectMainSW_PIN));
      }
    if (Set.SectMainSWType == 4) {
      if (!_digitalRead(Set.SectMainSW_PIN)) MainSWVal = SWON + 100; 
      if (!_digitalRead(Set.SectMainSW_PIN2))  MainSWVal = SWOFF - 100; 
      //MainSWVal = ((int(_digitalRead(Set.SectMainSW_PIN))*SWON)+(int(_digitalRead(Set.SectMainSW_PIN2))*SWOFF));
    } else if (Set.SectMainSWType == 5) {
      if (_digitalRead(Set.SectMainSW_PIN)) {
        MainSWVal = SWON + 100; 
      } else {
        MainSWVal = SWOFF - 100;
      }
    } else if (Set.SectMainSWType == 6) {
      if (!_digitalRead(Set.SectMainSW_PIN)) {
        MainSWVal = SWON + 100; 
      } else {
        MainSWVal = SWOFF - 100;
      ...
      MainSWVal = analogRead(Set.SectMainSW_PIN);
    }
  }
  if (Set.debugmodeSwitches) {
    Serial.print(" Main:[");
  ...
  if (Set.SectMainSWType == 2) {
    ...
    if (MainSWVal < Set.HitchLevelVal) {
      MainSWVal = SWON + 100;
    } else {
      MainSWVal = SWOFF - 100;
    }
    if (MainSWVal == MainSWValOld) { MainSWVal = ((SWOFF + SWON) / 2); }  //set "switch" to middle, when same status as last time
      ...
      MainSWValOld = MainSWVal;
    }  //first time when changed
  }
  ...
  if (MainSWVal > SWON) {
    SectMainOn = true;
    SectMainSWpressed = true;
    SectMainSWlastTime = SectSWcurrentTime;
  }
  if (MainSWVal < SWOFF) {
    SectMainOn = false;
    SectMainSWpressed = true;
  ...
  if (!SectMainSWpressed && SectAuto) {
    if ((SectGrFromAOG[0] > 0) || (SectGrFromAOG[1] > 0)) {
      SectMainOn = true;
  ...
  if (Set.SectNum > 8) {
    SectNumLoop = Set.SectNum - 8;
    a = 1;
  } else {
    SectNumLoop = Set.SectNum;
    a = 0;
  }
  do {
    for (int i = 0; i < SectNumLoop; i++) {
      if (SectSWVal[i + a * 8] == HIGH) {
        ...
        if (SectAuto) {
          ...
          if (bitRead(SectGrFromAOG[a], i) == HIGH) {
            bitSet(RelayOUT[a], i);
  ...
  if (Set.aogVersion == 17) {
    if (!SectAuto) {
      SCToAOG[6] = RelayOUT[0];
      SCToAOG[5] = RelayOUT[1];
    } else {
      SCToAOG[6] = 0;
      SCToAOG[5] = 0;
    }
    ...
    if (!SectAuto) {
      SCToAOG[9] = RelayOUT[0];
      SCToAOG[11] = RelayOUT[1];
    } else {
      SCToAOG[9] = 0;
      SCToAOG[11] = 0;
    }
  ...
  if (!SectAuto && !SectMainOn) {
    if (Set.SectNum > 8) {
      ...
      for (byte k = 0; k < Set.SectNum; k++) { bitSet(SectSWOffToAOG[0], k); }
  }
  ...
  if (SectAutoSWpressed && SectAuto && SectMainOn) {
    bitSet(SectMainToAOG, 0);
    RelayOUT[0] = RelayOUTOld[0];
    RelayOUT[1] = RelayOUTOld[1];
  }
  ...
  if (!SectAutoSWpressed) {
    RelayOUTOld[0] = RelayOUT[0];
    RelayOUTOld[1] = RelayOUT[1];
  }

  //if AUTO/Man SW moved to MAN, send Main on to AOG to turn all off
  if (SectAutoSWpressed && !SectAuto && !SectMainOn) {
    bitClear(SectMainToAOG, 0);
    bitSet(SectMainToAOG, 1);
    RelayOUT[0] = 0;
    RelayOUT[1] = 0;
  }
}  //end of SectSWRead

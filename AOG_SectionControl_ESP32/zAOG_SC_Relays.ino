void SetRelays(void)
{
	if (Set.SectNum > 8) {
		for (byte i = 0; i < 8; i++)
		{
			if (bitRead(RelayOUT[0], i) == LOW) {
				_digitalWrite(Set.Relay_PIN[i], !Set.SectRelaysON);
			}
			else {
				_digitalWrite(Set.Relay_PIN[i], Set.SectRelaysON);
			}
	...
	if (Set.Relais_MainValve_PIN != 255) {//main valve
		if ((RelayOUT[0] > 0) || (RelayOUT[1] > 0)) { _digitalWrite(Set.Relais_MainValve_PIN, Set.SectRelaysON); }
		else { _digitalWrite(Set.Relais_MainValve_PIN, !Set.SectRelaysON); }
	}
}
